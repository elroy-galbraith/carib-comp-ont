---
class: Regulator
id: dpa2020_ico
label: Information Commissioner (Jamaica)
uri: https://ontology.carib-comp.org/compliance/entity/dpa2020_ico
source_document: dpa2020
source_section: "§5"
source_text: >
  There shall be an Information Commissioner who shall be appointed by the
  Governor-General on the advice of the Prime Minister after consultation with
  the Leader of the Opposition.
properties:
  partOfStatute: "[[dpa2020]]"
prompt_version: manual-v1
model_snapshot: hand-curated
validation: PASS
---

## Information Commissioner (Jamaica)

**Established under** §5 of the Data Protection Act 2020; referenced in the §2 definition of "Commissioner".

> There shall be an Information Commissioner who shall be appointed by the
> Governor-General on the advice of the Prime Minister after consultation with
> the Leader of the Opposition.

### Mandate

The Information Commissioner is the independent regulator responsible for:

- Supervising compliance with the DPA 2020
- Investigating complaints from data subjects
- Issuing enforcement notices and monetary penalties
- Promoting public awareness of data protection rights
- Maintaining a register of data controllers

### FIBO alignment

`cco:Regulator` is declared `rdfs:subClassOf fibo-be-ge-ge:GovernmentalAuthority`.
The Information Commissioner is a statutory office and fits this alignment cleanly.
